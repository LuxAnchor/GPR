  ...
  const loadPhoto = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData?.error?.includes('需要')) {
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await fetch('/api/photos/public', {
        ...
        body: JSON.stringify({ code, view_code: viewCode, name: nameInput }),
      });
      ...
      setError(err.message || '验证失败，请检查密码和姓名');
    } finally {
      setIsVerifying(false);
    }
  };

  const getFacePosition = (face: Face) => {
    if (!imageRef.current) return null;

    const rect = imageRef.current.getBoundingClientRect();
    const centerX = ((face.x + face.width / 2) / imageRef.current.naturalWidth) * rect.width;
    const centerY = ((face.y + face.height / 2) / imageRef.current.naturalHeight) * rect.height;
    const size = Math.min(
      (face.width / imageRef.current.naturalWidth) * rect.width,
      (face.height / imageRef.current.naturalHeight) * rect.height
    );
              ...
              return (
                <div
                  ...
                  className={`face-hotspot absolute rounded-full cursor-pointer transition-all duration-300 ${
                    isSelected ? 'bg-orange-500/50 scale-125 ring-4 ring-orange-500' : 'bg-blue-500/20 hover:bg-blue-500/40'
                  }`}
                  style={{
                    left: pos.left,
                    top: pos.top,
          ...
          {showNameList && (
            <div className="bg-gray-50 px-6 py-8">
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 rounded-lg border-2 transition-all cursor-pointer ${
                        isSelected ? 'border-orange-500 bg-orange-50 shadow-md' : 'border-gray-200 bg-white hover:border-blue-400 hover:shadow'
                      }`}
                      onClick={() => {
                        setSelectedFaceId(isSelected ? null : face.id);
                        if (!isSelected) {
              ...
              {sortedFaces.length === 0 && (
                <div className="text-center text-gray-500 py-8">暂无标注名单</div>
              )}
            </div>
          )}
        </div>
