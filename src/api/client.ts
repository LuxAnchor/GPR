...
export const api = {
  ...
  lockPhoto: async (id: string): Promise<void> => {
    const response = await fetch(`${API_BASE}/${id}/lock`, {
      method: 'PUT',
      headers: getHeaders(),
    });
  ...
  updateDisplayName: async (id: string, displayName: string): Promise<void> => {
    const response = await fetch(`${API_BASE}/${id}/display-name`, {
      method: 'PUT',
  ...
  setViewCode: async (id: string, viewCode: string | null): Promise<void> => {
    const response = await fetch(`${API_BASE}/${id}/view-code`, {
      method: 'PUT',
      headers: getHeaders('application/json'),
  ...
  setAnnotateViewCode: async (id: string, annotateViewCode: string | null): Promise<void> => {
    const response = await fetch(`${API_BASE}/${id}/annotate-view-code`, {
      method: 'PUT',
      headers: getHeaders('application/json'),
      body: JSON.stringify({ annotateViewCode }),
    });
  ...
  regenerateAnnotateCode: async (id: string): Promise<string> => {
    const response = await fetch(`${API_BASE}/${id}/regenerate-annotate-code`, {
      method: 'PUT',
      headers: getHeaders(),
    });
  ...
  verifyPublicPhoto: async (code: string, viewCode?: string, name?: string): Promise<PhotoWithFaces> => {
    const response = await fetch(`${API_BASE}/public/verify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code, viewCode, name }),
    });
    if (!response.ok) {
      const error = await response.json();
  ...
  verifyAnnotatePhoto: async (annotateCode: string, annotateViewCode?: string): Promise<PhotoWithFaces> => {
    const response = await fetch(`${API_BASE}/annotate/verify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ annotateCode, annotateViewCode }),
    });
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || '验证失败');
  ...
  getPublicPhoto: async (code: string): Promise<PhotoWithFaces> => {
    const response = await fetch(`${API_BASE}/public/${code}`);
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || '获取照片失败');
    }
  ...
  getAnnotatePhoto: async (code: string): Promise<PhotoWithFaces> => {
    const response = await fetch(`${API_BASE}/annotate/${code}`);
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || '获取照片失败');
    }
